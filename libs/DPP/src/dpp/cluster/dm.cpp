...
void cluster::create_dm_channel(snowflake user_id, command_completion_event_t callback) {
	rest_request<channel>(this, API_PATH "/users", "@me", "channels", m_post, json({{"recipient_id", std::to_string(user_id)}}).dump(-1, ' ', false, json::error_handler_t::replace), [user_id, this, callback](const dpp::confirmation_callback_t &event) {
		if (!event.is_error()) {
			auto c = event.get<channel>();
			set_dm_channel(user_id, c.id);
		}
		callback(event);
	});
}
